...
for i in $(find /dev/snd -type c -print 2>/dev/null)
do
    AUDIO_UID=$(stat -c '%u' "${i}")
    AUDIO_GID=$(stat -c '%g' "${i}")
    # check if user matches device
    if id -u ${USER} | grep -qw "${AUDIO_UID}"; then
        echo "---permissions for ${i} are good---"
    else
        # check if group matches and that device has group rw
        if id -G ${USER} | grep -qw "${AUDIO_GID}" && [ $(stat -c '%A' "${i}" | cut -b 5,6) = "rw" ]; then
            echo "---permissions for ${i} are good---"
        # check if device needs to be added to video group
        elif ! id -G ${USER} | grep -qw "${AUDIO_GID}"; then
